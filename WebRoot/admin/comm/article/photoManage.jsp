...
<html>
							...
							items : [
								{
									...
									border : false,
									fileSize : 300,//限制文件大小
									uploadUrl : 'admin/fileOperate!fileUpload.action',
									flashUrl : 'js/swfupload.swf',//相对于jsp文件的目录
									filePostName : 'filedata', //后台接收参数
									fileTypes : '*.jpg;*.gif;*.png',//可上传文件类型
									postParams : {savePath:'${articleSrc.albumspath}',isFilenameNew:1} //上传文件存放目录
								}
							],listeners:{ 
								"close":function(){ 
	...
	<body>
		<form action="admin/article!photoManage.action" method="post" name="form_picManage" id="form_picManage">
			<input type="hidden" name="articlesrcid" id="articlesrcid" value="${articleSrc.articlesrcid }"/>
		</form>
		<div class="maindiv">
			<h1>
				学习频道管理&mdash;&mdash; ${activity.activitytitle } &mdash;&mdash;
				相册管理
			</h1>
		</div>
		...
		<table width="98%" border="0" cellpadding="2" cellspacing="1"
			...
			<tr align="center" bgcolor="#e8f0f3" height="22">
				<td colspan="5">
					<form action="admin/fileOperate!deleteFile.action" method="post" name="formDeleteFile" id="formDeleteFile">
