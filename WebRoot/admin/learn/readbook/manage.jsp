...
<html>
	...
	<body>
		<div class="maindiv">
			<h1>
				学习频道管理&mdash;&mdash;博览群书管理
			</h1>
		</div>

		<form action="admin/learn/readbook!openReadbook.action" method="post"
			...
			<table width="98%" border="0" cellpadding="2" cellspacing="1"
					...
					<td colspan="8" bgcolor="#FFFFFF" align="center">
						<table>
							<tr>
								...
								<td>
									<select name="srctypeid" id="srctypeid">
										<option value="">
											所有
										</option>
										<s:iterator value="readsrctypeAll" id="r">
											<option value="${r.srctypeid }"
												<c:if test="${r.srctypeid==param.srctypeid }">selected</c:if>>
												${r.srctype }
											</option>
										</s:iterator>
									</select>
				...
				<s:iterator value="#request.list_readSrc" id="r">
					<tr align="center" bgcolor="#FFFFFF"
						...
						<td>
							<u><c:if test="${r.isrecommend==1 }">是</c:if> <c:if
									test="${r.isrecommend==0 }">否</c:if> </u>
						</td>
						<td>
							<u><c:if test="${r.ishot==1 }">是</c:if> <c:if
									test="${r.ishot==0 }">否</c:if> </u>
						</td>
						<td>
						...
						<td>
							<a
								href="admin/learn/readbook!openModifyReadbook.action?readsrcid=${r.readsrcid }">修改</a>&nbsp;
							|&nbsp;
							<a
								href="admin/learn/readbook!deleteReadbook.action?readsrcid=${r.readsrcid }"
