...
<body>
	...
	<div class="mainbody">
  		<div class="seat"><a href="#">952116综合信息门户网首页</a> > <a href="<%=basePath%>Article/Show_News_Index.action">资讯频道</a> > <span>${articleSrc.articleType.articletypename}</span>><span>${articleSrc.articletitle }</span></div>
	</div>
<div class="mainbody">
  <div class="listbig">
    <div class="listleft">
      ...
      <div class="maind_1">${fn:substring(articleSrc.articletitle , 0,30)}<s:if test="%{articletitle.length()>30}">...</s:if></div>
      <div class="maind_2">
      	<span class="s1">${articleSrc.artfrom}&nbsp;&nbsp;&nbsp;&nbsp;<fmt:formatDate value="${articleSrc.createdate}" pattern="yyyy年MM月-dd日 kk:mm"/></span>
      	 	<span class="s2"><a href="#" id="a_share"><img src="img/learning_img/fen_1.gif" /></a><s:property value="articleSrc.sharenum" /></span>
       		<span class="s3"><a href="#" id="a_collect"><img src="img/learning_img/fen_2.gif" /></a><s:property value="articleSrc.collectionnum" /></span>
	  </div>
      <div class="maind_3">
      	<div class="ppmm">  	
        	  <p>
      ...
      <div class="maind_4">
 		<c:if test="${empty articleList}">
        	<center>没有相关内容</center>
        </c:if>
        <c:if test="${not empty articleList}">
        <ul>
          <li class="bbt">相关阅读：</li>
          <s:iterator value="#request.articleList" id="row">
          <li><a href="<%=basePath %>Article/getArticleById.action?id=${row.articlesrcid}">${row.articletitle}</a> <span><fmt:formatDate value="${row.createdate}" pattern="yyyy-MM-dd"/></span></li>
          </s:iterator>      
        </ul>
     </c:if>
      </div>
