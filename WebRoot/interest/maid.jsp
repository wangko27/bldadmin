...
<div class="mainbody">
  <div class="seat"><a href="#">952116综合信息门户网首页</a> > <a href="interest/XQ_index.action">兴趣</a> ><span><a href="interest/works_showActionWorks.action?activitId=${activity.activityid}&activitPro=${activity.programa.proID}">${activity.activitytitle}</span></a> > <span><a href="interest/list.action?id=${article.articleType.articletypeid}&activitPro=${activity.activityid}"><s:property value="article.articleType.articletypename" /></a></span></div>
</div>
<div class="mainbody">
  <div class="listbig">
    <div class="listleft">
      <div class="maind_1"><s:property value="article.articletitle" /></div>
      <div class="maind_2"><span class="s1"><fmt:formatDate value="${article.createdate}" pattern="yyyy-MM-dd"/></span>     
      ...
      <div class="maind_4">
        <ul>
          ...
          <li><a href="<%=basePath %>interest/viewById.action?id=${articleid}">${row.articletitle}</a> <span><fmt:formatDate value="${row.createdate}" pattern="yyyy-MM-dd"/></span></li>
          </s:iterator>   
        </ul>
      </div>
    </div>
