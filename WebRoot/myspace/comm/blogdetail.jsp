    ...
    <div class="mainbody_1">
	    ...
	    <div class="uuop">
	      ...
	      <div class="right">
	        ...
	        <div class="dtai">
	          <ul>
		          ...
		          <s:elseif test="blogtype == 2">
		            ...
		            <li><a href="myspace/comm/blogList.action?blogtype=3">大家的博文</li>
		          </s:elseif>
	        ...
	        <div class="boo_1">
	        <span>
		        <a href="myspace/comm/blogList.action?blogtype=${blogtype }">返回列表>></a> | 
		        <s:if test='blog.blogpre'><a href="myspace/comm/blogPre.action?blog.createdate=${blog.createdate }&&blogtype=${blogtype }">上一篇</a></s:if><s:else>上一篇</s:else>
		        <s:if test="blog.blognext"><a href="myspace/comm/blogNext.action?blog.createdate=${blog.createdate }&&blogtype=${blogtype }">下一篇</a></s:if><s:else>下一篇</s:else>
	         </span>
	          <span style=" float:right">
	          <s:if test="blogtype == 1">
		          <a href="myspace/comm/blogUpEdit.action?blog.blogid=${blog.blogid }">编辑</a> | 
		          <a href="myspace/comm/blogDel.action?blog.blogid=${blog.blogid }">删除</a>
		      </s:if>
	          </span>
	          </div>
	        <div class="boo_2">
	          ...
	          <h2>发表人：
	          <s:if test="blog.member.memberid == session.loginMemberId">${blog.member.nikename }</s:if>
	          <s:else><a href="Zone/index.action?TTid=${blog.member.memberid}">${blog.member.nikename }</a></s:else>
	          &nbsp;&nbsp;发表于：${blog.createdate } | 阅读：(${blog.readnum }) 评论：(<span id="comnum">${blog.commentnum }</span>) </h2>
	         ${blog.blogcontent }
	        </div>
