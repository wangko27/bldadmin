...
<html>
  <head>
    ...
    <title>您好，欢迎来到952116综合信息门户网！-- 我的动态</title>
				...
				$.ajax({
					...
					success:function(msg){
						if(msg == 'exception'){
							...
							window.location.href = "<%=basePath%>openzone/zoneIndexAction!myself.action";
						}
					}
				});
			});
        ...
        <div class="rzhong">
         <div class="xinqing">
          ...
          <div class="shukk">
	            <ul>
		         <li>
		           <textarea name="mood.moodtext" id="moodContent" cols="" rows="" onfocus="onblurMood(this)" style="color: black" >可输入200个字</textarea>
		         </li>
          ...
          <div class="dtai">
            <ul>
              ...
              <li class="bai"><a href ="<%=basePath%>openzone/zoneIndexAction!myself.action">我的动态</a></li>
          ...
          <div class="dongtai">
          	...
          	<c:choose>
          		<c:when test="${record.actiontype eq 1}">
          			<ul>
		            	<li class="tuuser"><a href="#"><img  <c:if test="${not empty record.member.headpath}">src="<%=basePath%>${record.member.headpath }"</c:if> <c:if test="${empty record.member.headpath}">src="<%=basePath%>img/user_img/imgmo.gif"</c:if> /></a></li>
		                <li class="tuuser1">
		                	<p class="tuzi1"><a href="#">${record.username}</a>${record.actiontitle}<a href="#">${record.album.albumname}</a></p>
		                    <p class="tuzi2"><a href="#"><img <c:if test="${not empty record.photo.photopath}">src="<%=basePath%>${record.photo.photopath}"</c:if> <c:if test="${empty record.photo.photopath}">src="<%=basePath%>img/user_img/imgmo.gif"</c:if> /></a></p>
          		...
          		<c:when test="${record.actiontype eq 2}">
          			 <ul>
		            	<li class="tuuser"><a href="#"><img <c:if test="${not empty record.member.headpath}">src="<%=basePath%>${record.member.headpath }"</c:if> <c:if test="${empty record.member.headpath}">src="<%=basePath%>img/user_img/imgmo.gif"</c:if> /></a></li>
		                <li class="tuuser1">
		                	<p class="tuzi1"><a href="#">${record.username}</a>${record.actiontitle}<a href="#"><c:if test="${fn:length(record.blog.blogtitle) > 20}">${record.blog.blogtitle}...</c:if><c:if test="${fn:length(record.blog.blogtitle) <= 20}">${record.blog.blogtitle}</c:if></a></p>
          		...
          		<c:when test="${record.actiontype eq 3}">
          			<ul>
		            	<li class="tuuser"><a href="#"><img <c:if test="${not empty record.member.headpath}">src="<%=basePath%>${record.member.headpath }"</c:if> <c:if test="${empty record.member.headpath}">src="<%=basePath%>img/user_img/imgmo.gif"</c:if>/></a></li>
		                <li class="tuuser1">
		                	<p class="tuzi1"><a href="#">${record.username}</a>${record.actiontitle}<span>${record.mood.moodtext}</span></p>
		                    <p class="time"><fmt:formatDate value="${record.actiondate}" pattern="yyyy-MM-dd hh:mm"/></p>
		                </li>
		            </ul>
          		</c:when>
